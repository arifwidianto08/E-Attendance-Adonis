...
class AuthController extends BaseController {
  ...
  async socialLogin({ request, response, auth, ally, params }) {
    ...
    let socialUser = null;
    try {
      socialUser = await ally
        .driver(social)
        .getUserByToken(socialToken, clientSecret);
    } catch (error) {
      console.log(error);
  ...
  async sendVerification({ request, response }) {
    ...
    if (!user) {
      throw ResourceNotFoundException.invoke(
        `Can not find user with email "${request.input('email')}"`
      );
    }
    const verificationToken = crypto
      .createHash('sha256')
  ...
  async password({ request, response, auth }) {
    ...
    if (!check) {
      throw ValidateErrorException.invoke({
